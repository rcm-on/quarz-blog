...
> Esto es una **validación suave**, en **dos repos propios**: el ajuste (N=16) sale de este blog (Quartz), y el segundo —el proyecto en .NET del ejemplo `ISqlParser` de arriba— aporta el caso cualitativo, verificado a mano, no puntos de la curva. No es un benchmark ni una ley del coste: la regresión ajusta *estos* puntos, no predice el tuyo. Lo que sí prueba, y no es poco, es que **el 71,5× del titular no generaliza** a una pregunta cualquiera, y que puedo **confirmar que grep y grafo dieron la misma respuesta correcta** porque son repos que conozco. Para saber tu número, mídelo: por eso este lab te da el método, no una cifra para citar.
